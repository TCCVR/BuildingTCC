    ...
    private void Update() {
        if (Input.GetMouseButtonDown(0) && !EventSystem.current.IsPointerOverGameObject()) {
            ...
            if (Vector3.Distance(playerTransform.position, mouseWorldPosition)<maxBuildDistance) {
                var newPlacedBuilding = Instantiate(activeBuildingType.transform, mouseWorldPosition, transform.rotation * Quaternion.Euler(0, 45*counter, 0));
                newPlacedBuilding.transform.parent = PlayerCreatedScenario.transform;
            }
