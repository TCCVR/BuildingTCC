...
namespace SensorSystem {
    public class HeartbeatGraph :MonoBehaviour, ISensorSubscriber<HeartData, SimpleSerialEventArg> {
        ...
        public void ProcessData(HeartData dataSet) {
            valueList.InsertValue(dataSet.HeartBPM);
            for (int i = 0; i < valueList.MaxSize; i++) {
                UpdateValue(i, valueList[i]);
            }
        }

        public void SubscribeTo(ISensorHandler<HeartData, SimpleSerialEventArg> handler) {
        ...
        public void Subs_OnOnSensorParsedData(object sender, EventArgs eventArgs) {
            HeartData data = new HeartData { 
                HeartBPM = CJMCUSerialConnection.Instance.SingleDataSet.HeartBPM, 
                Timestamp = CJMCUSerialConnection.Instance.SingleDataSet.Timestamp
            };
            ProcessData(data);
        }

        public void Subs_OnSensorSentData(object sender, SimpleSerialEventArg eventArgs) { }

        public void Subs_OnSensorSentLineData(object sender, SimpleSerialEventArg eventArgs) { }
